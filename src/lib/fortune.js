...
- 풍부함과 깊이(가장 중요): 표면 묘사에서 멈추지 말고 '왜 그런지'의 까닭과 '그래서 어떻게 살아가면 좋은지'까지 한 겹 더 들어가세요. 각 항목에는 누구에게나 맞는 일반론이 아니라 이 명식에서만 길어낼 수 있는 통찰 한 가지를 반드시 담아, 읽는 사람이 '이건 진짜 내 이야기'라고 느끼게 하세요. 추상적 형용사를 늘리는 대신 구체적인 장면·감각·상황·관계로 그려 생생하고 밀도 있게 쓰세요.
- '어떻게 알았지?' 인지(돈 내고 볼 만한 핵심): 사람이 살면서 실제로 겪는 구체적 패턴을 콕 집어, 읽는 이가 '맞아, 나 진짜 이래' 하고 무릎을 치게 만드세요. 예: 돈이 모이다가도 어떤 순간 새어나가는 버릇, 관계에서 반복되는 특정 엇갈림, 일이 잘 풀리기 직전 스스로 제동을 거는 습관처럼 — 막연한 성격 묘사가 아니라 '겪어본 사람만 아는' 장면으로. 단, 단정·공포 조장은 금지하고 따뜻하게.
- 타이밍과 실행: 추상적 조언 대신 '언제(시기·나이대·계기)'와 '무엇을(구체적 한 가지 행동)'을 짝지어 주세요. 사람은 '나에 대한 정확한 묘사 + 지금 뭘 하면 되는지'에 지갑을 엽니다.
- 섹션 간 반복 금지(매우 중요): 그 사람의 가장 두드러진 약점 하나(예: 완벽주의, 마음의 벽, 감정 기복)를 모든 섹션의 결론으로 돌려막지 마세요. 성격·재능·재물·애정·직업·건강·조언은 각각 그 영역에서만 나오는 고유한 발견을 핵심으로 삼고, 같은 교훈('내려놓으세요'·'벽을 낮추세요'·'완벽하지 않아도 괜찮아요')이 리포트 전체에서 두 번 넘게 되풀이되지 않게 하세요. 위로의 마무리 문구도 섹션마다 다르게.`;
...
export function buildPrompt(mode, data, data2, userName, extra) {
  ...
  // fortune (오늘의 운세) — 기본
  return {
    system: `${base} 오늘(${today}) 운세를 분석합니다.${WARM}\n오늘의 길흉 점수는 ${extra?.scoreHint ?? 80}점이에요(일진 기준). 이 점수에 어울리는 톤·내용으로 일관되게 작성해 주세요.\n필드: oneliner(15자 이내), ohaengSummary(오행 기운 한 문장), comprehensive/wealthText/loveText/successText/healthText(각 2~3문장, 밀도 있게), actionTips{wealth,love,success,health 각 한 줄}, classic(오늘 ${userName}님께 건네는 동양 고전 명구 하나 — 사자성어·고사성어·불교 구절(반야심경 등)·사서삼경(논어·주역 등) 중 오늘 기운에 가장 어울리는 단 하나. 흔하고 진부한 건 피하기. hanja=원문 한자 4~10자, eum=한글 음, meaning=쉬운 우리말 뜻 한 문장, source=출처나 갈래 예 '논어'·'반야심경'·'고사성어'), timeFlow{morning,afternoon,evening 각 한 줄}, luckyColor, luckyNumber(1~99), luckyFood, luckyStyle(코디 한 줄), basisSummary(근거 2~3줄), score(1~100).
[중요·동어반복 금지] 다섯 분야(종합·재물·애정·성공·건강)는 서로 다른 구체적 장면·행동을 담고, 같은 위로의 말을 되풀이하지 마세요. '좋은 기운이 흐른다'·'긍정적인 마음으로'·'~수 있어요' 같은 막연하고 상투적인 표현 대신, 오늘 실제로 해볼 행동·상황·소재로 또렷하게. 문장 수를 채우려 비슷한 말을 늘이지 말고 짧고 밀도 있게.`,
...
- classic: ${userName}님의 명식 전체(강한 기운·부족한 기운·인생 흐름)를 한 구절로 품는 '동양 고전 명구' 단 하나를 고르세요. 사자성어·고사성어·사서삼경(논어·맹자·대학·중용·시경·서경·주역)·도덕경·장자·반야심경 등 어디서든 좋습니다. 누구에게나 흔한 진부한 구절(예: 새옹지마, 일체유심조)은 피하고, 이 사람에게 정말 들어맞는 구절을 신중히 고르세요. 다음을 채웁니다:
  · hanja(원문 한자만, 4~12자 정도로 너무 길지 않게)
  · eum(한글 음. 예: '적소성대')
  · meaning(쉬운 우리말 뜻 한 문장 — 직역 느낌으로 또렷하게)
  · source(출처나 갈래. 예: '주역 계사전', '논어 학이편', '반야심경', '노자 도덕경', '고사성어')
  · why(왜 이 구절이 ${userName}님의 명식과 삶에 어울리는지 따뜻하게 1문장 — 명식 근거를 가볍게 곁들여)`,
    user: `${userName}님, 일주 ${ilju}. 초년·중년·말년의 흐름, ${YEAR}년부터 향후 3년, 인생 조언, 맺음말, 그리고 명식에 어울리는 동양 고전 한 구절을 작성해 주세요.`,
    schema: S({
      lifeEarly: STR, lifeMid: STR, lifeLate: STR,
      years: AOBJ({ keyword: STR, text: STR, mark: STR }, ['keyword','text','mark']),
      advice: STR, closing: STR,
      classic: S({ hanja: STR, eum: STR, meaning: STR, source: STR, why: STR }, ['hanja','eum','meaning','source','why']),
    }, ['lifeEarly','lifeMid','lifeLate','years','advice','closing','classic']),
  };
}
...
export async function generateReport(birth, userName = '천문') {
  ...
  return {
    ...
    classic: future.classic,              // 동양 고전 명구(엔딩 '오늘 새길 한 구절')
    // 하위호환 별칭 (이메일 라이트 등)
